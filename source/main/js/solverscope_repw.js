...
function repw_folder_share ( folder_id, parent ) {
	...
	$.ajax({
		...
		success: function( data ) {
			...
			for ( i = 0; i < 6; i++ ) {
				...
				tx += '<tr>';
				tx += '<td width="100">' + tit + '</td>';
				tx += '<td>';
		
				tx += '<div class="folder-flow-r" id="folder-flow-r-' + i + '">';
				...
				tx += '<div style="display:none" class="folder-flow-w" id="folder-flow-w-' + i + '">';
				tx += '<input style="width:15rem" id="folder-flow-input-' + i + '" onkeyup="rep_folder_flow_search(' + i + ')">';
				tx += '<div style="position:absolute;width:14rem;" id="folder-flow-select-' + i + '"></div>';
			...
			$( '#myModalTitle' ).html( svc_lang_str( 'PROMPT_FOLDER_FLOW' ) );
			$( '#myModalBody' ).html( tx );
			$( '#myModalButton' ).html( '' );
			$( '#myModal' ).modal( 'show' );
...
function repw_folder_share_update ( op ) {
	$( '.folder-flow-r' ).show();
	$( '.folder-flow-w' ).hide();
	$( '#folder-flow-input-' + op ).val( '' );
	$( '#folder-flow-select-' + op ).html( '' );
...
function rep_folder_flow_search( op ) {
	var str = $( '#folder-flow-input-' + op ).val();
	str = str.trim().toLowerCase();
	str = svc_remove_accents( str );
	
	if ( str.length > 2 ) {
		rows = global_sys_person_domain_list.filter( arr => arr.PERSON_NAME_CLEAN.toLowerCase().indexOf( str ) > -1 );
		var tx = '';
			if ( rows.length > 0 ) {
			tx += '<select id="folder-flow-option-' + op + '" size="10" style="width:100%">';
			for ( var i = 0; i < rows.length; i++ ){
				tx += '<option value="' + rows[i]['PERSON_ID'] +'">' + rows[i]['PERSON_NAME'] + '</option>';
			}
			tx += '</select>';
		}
		$( '#folder-flow-select-' + op ).html( tx );
	}
	else {
		$( '#folder-flow-select-' + op ).html( '' );
	}
...
function repw_folder_share_save ( folder_id, op ) {
	var person2_id = $( '#folder-flow-option-' + op ).val();
	if ( person2_id == undefined ) person2_id = 0;
	$.ajax({
		url: 'app/',
		type: 'POST',
...
function repw_quiite_import( folder_id ) {

	if ( confirm( svc_lang_str( 'CONFIRM_QUIITE_IMPORT' ) ) ) {
		
		$( '#myModalTitle' ).html( svc_lang_str( 'PROMPT_FILE_NAME' ) );
		$( '#myModalBody' ).html( '<input type="file" id="excel-file">' );
		$( '#myModalButton' ).html( '<button type="button" class="btn btn-primary" onclick="repw_quiite_import_save(' + folder_id + ')">' + svc_lang_str( 'SAVE' ) + '</button>' );
		$( '#myModal' ).modal( 'show' );
...
function repw_quiite_import_save( folder_id ) {	

	var excel_file = $( '#excel-file' ).prop( 'files' )[0];
	

	
	if ( excel_file.type.indexOf( 'officedocument.spreadsheet' ) < 0 ) {
		...
		form_data.append( 'person_id', global_person_id );
		form_data.append( 'folder_id', folder_id );

		//upload image
		$.ajax({
		    url: 'app/procedures/rep/repw_quiite_import.php',
		    data: form_data,
		    type: 'POST',
		    contentType: false,
		    processData: false,
			success: function( data ){
				...
				if ( data.trim() == 'OK' ) {
					...
					alert( svc_lang_str( 'ERROR_QUIITE_IMPORT: ' ) + data.trim() );
				}
...
//view items from current folder
function repw_quiait_insert_this( quiasm_id, object_id ) {
	...
	$.ajax({
		...
		success: function( data ) {
			...
			repw_quiait_insert_folder_list( quiasm_id, object_id, object['OBJECT_FOLDER_ID'] );
		}
	});
...
function repw_quiait_insert_other( quiasm_id, object_id, folder_id ) {
	...
	$.ajax({
		...
		success: function( data ) {
			...
			if ( rows.length == 0 ) {
				alert( svc_lang_str( 'NOT_SUB_FOLDER' ) );
			}
			else {
				var tx = '';
				
				if ( folder_id > 0 ) tx += '<button class="btn btn-info btn-xs" onclick="repw_quiait_insert_other(' + quiasm_id + ',' + object_id + ', 0)"><i class="fa fa-home"></i></button><br>&nbsp;<br>';

				for ( var i = 0; i < rows.length ; i++ ) {
					tx += '<input type="radio" name="folder" class="mr-3" value="' + rows[i]['FOLDER_ID'] + '">';
					tx += '<a href="#" onclick="repw_quiait_insert_other(' + quiasm_id + ',' + object_id + ',' + rows[i]['FOLDER_ID'] + ')">' + rows[i]['FOLDER_NAME'] + '</a>';
					if ( global_master == 1 ) tx += ' <span class="svc-master">FOLDER_ID: ' + rows[i]['FOLDER_ID'] + '</span>';
					tx += '<br>';
				}
				$( '#myModalTitle' ).html( svc_lang_str( 'SELECT_FOLDER' ) );
				$( '#myModalBody' ).html( tx );
				$( '#myModalButton' ).html( '<button type="button" class="btn btn-primary" onclick="repw_quiait_insert_other_view( ' + quiasm_id + ', ' + object_id + ' )">' + svc_lang_str( 'VIEW_FOLDER' ) + '</button>' );
				$( '#myModal' ).modal( 'show' );
...
//view items from another folder
function repw_quiait_insert_other_view( quiasm_id, object_id ) {

	var folder_id = $('input[name=folder]:checked').val();
	if ( folder_id != undefined ) {
		repw_quiait_insert_folder_list( quiasm_id, object_id, folder_id );
	}

}


//list items from selected folder
function repw_quiait_insert_folder_list( quiasm_id, object_id, folder_id ) {
	
	$.ajax({
		url: 'app/',
		type: 'POST',
		headers: { 'tk': tk, 'procedure': 'rep_quiite_list' },
		data: { 'folder_id': folder_id },
		success: function( data ) {
			var rows = svc_get_json( data );
			...
			tx += '<tbody>';
			tx += '<tr>';
			tx += '<td><input type="checkbox" checked id="check-all" onclick="$( \'.check-item\' ).not(this).prop(\'checked\', this.checked);"></td>';
			tx += '<td><b>' + svc_lang_str( 'ALL_ITEMS' ) + '</b></td>' ;
			tx += '</tr>';
			for ( var i = 0; i < rows.length ; i++ ) {
				tx += '<tr>';
			...
			$( '#myModalTitle' ).html( svc_lang_str( 'SELECT_ITEMS' ) );
			$( '#myModalBody' ).html( tx );
			$( '#myModalButton' ).html( '<button type="button" class="btn btn-primary" onclick="repw_quiait_insert_folder_save( ' + quiasm_id + ', ' + object_id + ' )">' + svc_lang_str( 'SAVE' ) + '</button>' );
			$( '#myModal' ).modal( 'show' );
...
//insert items from selected folder
...
//input ID to insert item
function repw_quiait_insert_id( quiasm_id, object_id ) {
	...
	$( '#myModalTitle' ).html( svc_lang_str( 'PROMPT_ID' ) );
	$( '#myModalBody' ).html( tx );
	$( '#myModalButton' ).html( '<button type="button" class="btn btn-primary" onclick="repw_quiait_insert_id_save( ' + quiasm_id + ', ' + object_id + ' )">' + svc_lang_str( 'SAVE' ) + '</button>' );
	$( '#myModal' ).modal( 'show' );
